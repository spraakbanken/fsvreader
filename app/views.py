#from app import app, karp_query, serve_static_page, render_template
import codecs
from flask import jsonify, render_template, Flask, url_for
...
def lookup(words):
    ...
    try:
        ...
        return jsonify({"call": karp_q, 'words': words, 'splitted': '|'.join(words.split('--'))})
    try:
        ...
        return jsonify({"error": "%s" % e, "called": karp_q, 'words': words, 'hits': res.get('hits',{}).get('hits')})
    ...
    return render_template('lex.html', hword=wordlist[0][0], words=wordlist, data=worddata, hits=res.get("hits", {}).get("total", 0))

@app.route('/')
def main():
    # move to config
    APP_ROOT = os.path.dirname(os.path.abspath(__file__))   # refers to application_top
    APP_STATIC = os.path.join(APP_ROOT, 'pages')
...
def showtext(dirname, filename):
    ...
    #return render_template('reader.html', textframe='fsvtext.html',
    #                       lexframe="/fsvreader/lexseasy/")
